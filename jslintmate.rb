...
if ENV['TM_FILEPATH']
  ...
  lint.gsub!(/^(Lint at line )(\d+)(.+?:)(.+?)\n(?:(.+?)\n\n)?/m) do
    line_uri = "txmt://open?url=file://#{ENV['TM_FILEPATH']}" <<
      "&line=#{CGI.escapeHTML($2)}"
    %Q{
      <p>
        <strong>
          #{CGI.escapeHTML($1)}
          <a href="#{line_uri}">#{CGI.escapeHTML($2)}</a>
          #{CGI.escapeHTML($3)}
        </strong>
        #{CGI.escapeHTML($4)}
  ...
  if lint =~ /No problems found/
    ...
    result = %Q{
      <header>
        Lint found in: <span class="filepath">TM_FILEPATH</span>
      </header>
      <div class="lint">#{lint}</div>
    }
  end
...
<head>
  ...
  <style>
    ...
    pre {
      margin-top: 2px;
      padding: 2px 4px;
      background: -webkit-gradient(linear, left top, left bottom, from(rgba(0, 0, 0, 0.1)), to(rgba(0, 0, 0, 0.05)), color-stop(0.1, rgba(0, 0, 0, 0.05)));
      background: -webkit-linear-gradient(top, rgba(0, 0, 0, 0.1), 0.1 rgba(0, 0, 0, 0.05), rgba(0, 0, 0, 0.05));
      background:         linear-gradient(top, rgba(0, 0, 0, 0.1), 0.1 rgba(0, 0, 0, 0.05), rgba(0, 0, 0, 0.05));
    ...
    header {
      ...
      width: auto;
      padding: 0.6667em;
      background: -webkit-gradient(linear, left top, left bottom, from(#333), to(#090909), color-stop(0.5, #191919), color-stop(0.5, #090909));
